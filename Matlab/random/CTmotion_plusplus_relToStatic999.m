clear all; close all;
% Function: Matching bone(fragements) onto bone from static scan using ICP
...
dir = 'C:\Users\u0078973\Documents\KULAK Lokaal\PhD\Matlab\Code\4D_CT\Example data\611\'; % Folder with files
data_available =1; % loading data if available (=1), else, generate data
    filename = 'RotTransData_tovStat8.txt';
    file = [dir filename];
radius_axis_available =1; % loading data if available (=1), else, generate data
    filenameAX = 'RadAxes_tovStat.txt';
    fileAX = [dir filenameAX];
inertial_axis_available =1; % loading data if available (=1), else, generate data
...
Rad_frag = '611L_phase8_rad.stl';
Rad_static = '611L_static_rad.stl';
MC_frag = '611L_phase8_mc1.stl';
MC_static = '611L_static_mc1.stl';
Sca1 = '611L_phase8_sca.stl';
Sca_static = '611L_static_sca.stl';
Trap1 = '611L_phase8_trp.stl';
...
% Transforming all data to radius CS (incl. transforming dynamic scan to static CS)
...
% Sca ifv lokaal RadiusAssenstelse
if(~data_available)
...
% MC1 expressed in relation tot the radius CS.
if(~data_available)
    [R_MC, T_MC, ERROR_MC] = RegisterBones(F_MC_end_init, MC1, F_MC, MC2,1);
end
Hom_MC = [R_MC, T_MC;0 0 0 1];
inv_Hom_MC = [R_MC.' -R_MC.'*T_MC; 0 0 0 1];
...
%             {ones(size(MC1,1),1),ones(size(MC2,1),1)});
    ...
    MC3 = (inv_Hom_MC * [MC2.' ; ones(1,size(MC2,1))]).';
    MC3 = MC3(:,1:3);
    
%     Calculation MC centroid displacement
% Calculating centroid motion
cent_MC2 = mean(MC2);
cent_MC3 = mean(MC3);
% Calculating centroid displacement (not realy necessary)
disp_MC23 = cent_MC3 - cent_MC2;
dist_MCTrap2 = cent_Trap1 - cent_MC2;
dist_MCTrap3 = cent_Trap2 - cent_MC3;
    
    h4 = figure;
    
    [obj, li, ax] = GUI_PlotShells(h4, {F_MC_end_init;F_MC;F_MC}, {MC1;MC3;MC2},...
            {ones(size(MC1,1),1),ones(size(MC3,1),1),ones(size(MC2,1),1)});
%     [obj, li, ax] = GUI_PlotShells(h4, {F_MC_end_init;F_MC}, {MC1;MC2},...
%             {ones(size(MC1,1),1),ones(size(MC2,1),1)});
