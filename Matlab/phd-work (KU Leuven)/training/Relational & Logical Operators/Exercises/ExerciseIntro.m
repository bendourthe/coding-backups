...
D = 1:10; 
h1 = D - (C <= 7);          % a) subtract from D taking 1 for C <= 7 and 0 otherwise 
h2 = (C >= 2) & (C < 4); 	% b) ones at positions where 2 <= C < 4 
h3 = (C == D) | D == 3; 	% c) ones at positions where C == D or C == 3 
h4 = any(C > 5); 			% d) 1 when ANY of the C elements are larger than 5  
h5 = all(D > 2); 			% e) 1 when ALL D-elements are larger that 2 
h6 = find(C);               % f) locate all nonzero elements of array C
...
year = 2013;
LeapYear2 = (rem(year,4) == 0) && (rem(year,100) ~= 0 || rem(year,400) == 0);
