function arrayData = createData(target, sensor)

% Copyright The MathWorks, Inc. 2008
...
for n=1:numTargets
    ...
    timeDelay = distance/target(n).signal.c             ;%Time delay to source
...
% Add noise
for k=1:sensor.numSamples
   arrayData(k,:)=noisyvar(arrayData(k,:).',sensor.SNR,numTargets).';
end
