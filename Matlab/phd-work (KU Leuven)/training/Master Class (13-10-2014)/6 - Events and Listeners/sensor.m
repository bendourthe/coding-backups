classdef sensor
    ...
    methods
        ...
        function obj   = registerTarget(obj,target)
            for n = 1:length(target)
                obj.targets{end+1} = target{n};
                
%                 obj.HLLm = addlistener(obj.FcnObject,'Lm','PostSet',...
%                     @(src,evnt)listenLm(obj,src,evnt)); % Add obj to argument list

                addlistener(target{n},'AoA','PostSet', ...
                    @(src, event)obj.listenerCallback(src, event));
            end
            obj.computeAoA(target);
        end
        
        %% Listener Callback
        function arrivingAngles = listenerCallback(obj, src, event)
            arrivingAngles = obj.computeAoA(obj.targets);
        end

        %% wavelength getter function
        function wavelength = get.wavelength(obj)
            wavelength = sensor.spdLight / obj.frequency;
        end
        
        %% spacing getter function
        function spacing = get.spacing(obj)
            spacing = obj.wavelength / 2;
        end
        
        %% Compute AoA method
        function arrivalAngles = computeAoA(  obj, target   )
            arrayData          = sensorSignal(obj, target   ) ;
            arrivalAngles      = determineAoA(obj, arrayData) ;
        end
    ...
    methods (Access = protected)
        ...
        function arrayData = sensorSignal(obj, target)
            arrayData = computeSensorSignal(obj, target) ;
        end
    end
end
