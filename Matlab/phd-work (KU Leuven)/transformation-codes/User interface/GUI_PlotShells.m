...
if isempty(colors)
    colors = num2cell(GUI_DistributeColors(length(F)),2);
elseif size(colors, 1)<length(F)
    colors = num2cell(colors(ones(length(F), 1),:),2); %Replicate single color
...
for ind = 1:length(F)
    if ~isempty(F{ind}) && ~isempty(V{ind})
        colors_tmp = colors{ind};
        if size(colors_tmp,1)==1
            obj(end+1,1) = patch('Parent', ax, ...
                                 'Vertices', V{ind}, 'Faces', F{ind}, ...
                                 'FaceColor', colors_tmp, ...
                                 'EdgeColor', edgecolor, ...
                                 'EdgeAlpha', edgealpha, ...
                                 'AmbientStrength', 0.4, ...
                                 'DiffuseStrength', 0.8, ...
