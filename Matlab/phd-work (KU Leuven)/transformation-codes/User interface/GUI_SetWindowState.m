function GUI_SetWindowState(h, state)
%GUI_SetWindowState  Maximimzes, minimizes or restores a figure.
%wb20070911
%
%   Syntax:
%    GUI_SetWindowState(fig, state)
%
%   Input:
%    fig:   Column vector of figure handles.
%    state: String indicating what to do. If set to 'maximize', the figures
%           will be maximized; 'minimize' minimizes the figures and
%           'restore' restores them. State can be an N-by-1 cell array of
%           strings, the elements of which correspond with the handles in
%           fig. If state is a single string, it will be applied to every
%           figure.
%
%   Output: none
%
%   Effect: This function will minimize, maximize or restore the figures
%   indicated by fig, depending on the string(s) in state. It does not work
%   on invisible figures (which have their 'Visible' property set to
%   'off').
%
%   Dependencies: none
%
...
%Contributed to Matlab file exchange on 05/01/2006 by Matthew Whitaker
%(matt@radimage.com).
%Downloaded from Matlab file exchange on 31/03/2006 by Ward Bartels.
%WB, 11/09/2007: Update from Matlab file exchange.
...
% %Original help text (wb20070911), begin:
% 
...
% 
% %Original help text (wb20070911), end.