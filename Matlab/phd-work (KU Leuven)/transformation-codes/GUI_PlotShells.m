...
%   Syntax:
%    [obj, li, ax] = ...
%        GUI_PlotShells(F, V, curvatures, colors, edgealpha, lit, ...
%           translucent, merge)
...
%                 empty cell array.
%    curvatures:  N-by-1 array containing curvatures at vertices. Each row
%                 corresponds to a vertex. If the mesh contains multiple 
%                 shells, the curvatures-matrices should be placed in an 
%                 N-by-1 cell array. Optional, defaults to an empty cell 
%                 array.
...
%Set defaults for F, V, curvatures, colors, edgealpha, lit, translucent and merge
defaults = {cell(0, 1) cell(0, 1) cell(0,1) zeros(0, 3) 0 [] false false};
argin(8:-1:nin+1) = defaults(8:-1:nin+1);

%Transfer inputs
[F, V, curvatures, colors, edgealpha, lit, translucent, merge] = argin{:};
...
for ind = 1:length(F)
    if ~isempty(F{ind}) && ~isempty(V{ind})
        obj(end+1,1) = patch('Parent', ax, ...
                             ...
                             'FaceVertexCData',colors(ind,:), ...
                             'FaceColor', colors(ind,:), ... %%%%%%%%%%%%%%%
                             'EdgeColor', edgecolor, ...
                             'EdgeAlpha', edgealpha, ...
                             'CDataMapping', 'scaled',...
                             'FaceVertexCData', curvatures{ind},...
                             'AmbientStrength', 0.4, ...
                             'DiffuseStrength', 0.8, ...
