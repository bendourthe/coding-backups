...
% Plots the MC1 with the new coordinate system
    fmc1 = figure;
    [obj, li, ax] = GUI_PlotShells(fmc1, {F_mc1_mc1cs},{V_mc1_mc1cs},...
...
sol = solve([Tt(i,1)*10^-3 == system(1), Tt(i,2)*10^-3 == system(2), Tt(i,3)*10^-3 == system(3)],[Rx,Ry,Rz])
...
Rot_z(i) = S.Rz;
    ...
    Rot_x_inv(i) = -(Tt(i,1)*10^-3/I1*dt^2);
    Rot_y_inv(i) = -(Tt(i,2)*10^-3/I2*dt^2);
    Rot_z_inv(i) = -(Tt(i,3)*10^-3/I3*dt^2);
        Roti_mc1_inv = [cos(Rot_z_inv(i))*cos(Rot_y_inv(i)),...
        cos(Rot_z_inv(i))*sin(Rot_y_inv(i))*sin(Rot_x_inv(i))-sin(Rot_z_inv(i))*cos(Rot_x_inv(i)),...
        cos(Rot_z_inv(i))*sin(Rot_y_inv(i))*cos(Rot_x_inv(i))+sin(Rot_z_inv(i))*sin(Rot_x_inv(i));...
        sin(Rot_z_inv(i))*cos(Rot_y_inv(i)),...
        sin(Rot_z_inv(i))*sin(Rot_y_inv(i))*sin(Rot_x_inv(i))+cos(Rot_z_inv(i))*cos(Rot_x_inv(i)),...
        sin(Rot_z_inv(i))*sin(Rot_y_inv(i))*cos(Rot_x_inv(i))-cos(Rot_z_inv(i))*sin(Rot_x_inv(i));...
        -sin(Rot_y_inv(i)), cos(Rot_y_inv(i))*sin(Rot_x_inv(i)), cos(Rot_y_inv(i))*cos(Rot_x_inv(i))];
