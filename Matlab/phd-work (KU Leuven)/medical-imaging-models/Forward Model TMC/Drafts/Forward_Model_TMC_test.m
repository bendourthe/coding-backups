...
z_angle = 0;
    ...
    % Ligament properties
AL = 0.5; % average cross-sectional area of thumb ligaments (in mm2)
C1 = 1; % in MPa
C2 = 1; % in MPa
C3 = 33; % in MPa
nu1 = 1; % in MPa.s
...
arrow([IPs(5,1);IPs(5,2);IPs(5,3)],[IPs(1,1);IPs(1,2);IPs(1,3)],...
        10,10,10,5,'EdgeColor','b','FaceColor','b');
set(gcf,'numbertitle','off','name','Visualization of the center of rotation');
...
for i=1:tmax*10
    ...
    Rot_x(i) = Tt(i,1)*10^-3/I1*t(i);
    Rot_y(i) = Tt(i,2)*10^-3/I2*t(i);
    Rot_z(i) = Tt(i,3)*10^-3/I3*t(i);
    % (degrees)
    ...
    Trans_CR = -CR(i,:).';
    Vi_mc1 = Vi_mc1.' + repmat(Trans_CR,1,size(Vi_mc1,1));
    Vi_mc1 = Vi_mc1.';    
...
f9=figure;
plot(t,Tt(:,1),'r',t,Tt(:,2),'g',t,Tt(:,3),'b');
xlabel('time (ms)');
ylabel('Torque (N.mm)')
...
f11=figure;
plot(t,Rot_deg_x,'r',t,Rot_deg_y,'g',t,Rot_deg_z,'b');
xlabel('time (ms)');
