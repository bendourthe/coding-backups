...
% MODEL PARAMETERS
    ...
    % Time frame
tmax = 1000; % in ms (equivalent to number of iteration)
...
z_angle = 0;
    ...
    % Ligament properties
th_SAOL = 1; % thickness of the SAOL at rest (mm)
w_SAOL = 15.3;  % width of the SAOL at rest (mm)
th_dAOL = 1;
w_dAOL = 7.19;
th_DRL = 0.9;
w_DRL = 5.9;
...
% INITIAL CONDITIONS
    ...
    vol_SAOL = linit_SAOL*th_SAOL*w_SAOL;
    vol_dAOL = linit_dAOL*th_dAOL*w_dAOL;
...
for i=1:length(CA_F_trap)
    ...
    % stress at P1
    stressP1_mc1(i) = 1/4*HA*(1 + d1*(Defmc1_P1(i) - 1))*1/Defmc1_P1(i)*(Defmc1_P1(i).^2 - 1/(Defmc1_P1(i).^2))+exp(Cc*(Defmc1_P1(i).^2 - 3));
    stressP1_trap(i) = 1/4*HA*(1 + d1*(Deftrap_P1(i) - 1))*1/Deftrap_P1(i)*(Deftrap_P1(i).^2 - 1/(Deftrap_P1(i).^2))+exp(Cc*(Deftrap_P1(i).^2 - 3));
    stressP1(i) = stressP1_mc1(i) + stressP1_trap(i);
    % stress at P2
    stressP2_mc1(i) = 1/4*HA*(1 + d1*(Defmc1_P2(i) - 1))*1/Defmc1_P2(i)*(Defmc1_P2(i).^2 - 1/(Defmc1_P2(i).^2))+exp(Cc*(Defmc1_P2(i).^2 - 3));
    stressP2_trap(i) = 1/4*HA*(1 + d1*(Deftrap_P2(i) - 1))*1/Deftrap_P2(i)*(Deftrap_P2(i).^2 - 1/(Deftrap_P2(i).^2))+exp(Cc*(Deftrap_P2(i).^2 - 3));
    stressP2(i) = stressP2_mc1(i) + stressP2_trap(i);
    % stress at P3
    stressP3_mc1(i) = 1/4*HA*(1 + d1*(Defmc1_P3(i) - 1))*1/Defmc1_P3(i)*(Defmc1_P3(i).^2 - 1/(Defmc1_P3(i).^2))+exp(Cc*(Defmc1_P3(i).^2 - 3));
    stressP3_trap(i) = 1/4*HA*(1 + d1*(Deftrap_P3(i) - 1))*1/Deftrap_P3(i)*(Deftrap_P3(i).^2 - 1/(Deftrap_P3(i).^2))+exp(Cc*(Deftrap_P3(i).^2 - 3));
    ...
    for i=1:length(CA_F_trap)
        MA_cart(i,:) = [OF(i,1)-CP(1) OF(i,2)-CP(2) OF(i,3)-CP(3)];
...
for i=1:tmax*10
        ...
        % LIGAMENTS
            ...
            CSA_SAOL(i) = vol_SAOL/l_SAOL(i);
            CSA_dAOL(i) = vol_dAOL/l_dAOL(i);
            ...
            % nonlinear viscoelastic stress in the stretched ligaments
                % SAOL
            if stretch_SAOL(i) > 1
                s_SAOL(i) = (1-Gam*exp(-Gbm*(stretch_SAOL(i).^2+2/stretch_SAOL(i)-3)))*...
                    (C1*(stretch_SAOL(i).^2-1/stretch_SAOL(i))+2*C2*(stretch_SAOL(i)-1/stretch_SAOL(i).^2))+...
                    2*(1-Gaf*exp(-Gbf*(stretch_SAOL(i).^2-1)))*C3*stretch_SAOL(i).^2*...
                    (exp(C4*(stretch_SAOL(i).^2-1))-1);
                s_SAOLi = s_SAOL(i);
            else
                s_SAOL(i) = s_SAOLi;
            end
                % dAOL
            if stretch_dAOL(i) > 1
                s_dAOL(i) = (1-Gam*exp(-Gbm*(stretch_dAOL(i).^2+2/stretch_dAOL(i)-3)))*...
                    (C1*(stretch_dAOL(i).^2-1/stretch_dAOL(i))+2*C2*(stretch_dAOL(i)-1/stretch_dAOL(i).^2))+...
                    2*(1-Gaf*exp(-Gbf*(stretch_dAOL(i).^2-1)))*C3*stretch_dAOL(i).^2*...
                    (exp(C4*(stretch_dAOL(i).^2-1))-1);
                s_dAOLi = s_dAOL(i);
            else
                s_dAOL(i) = s_dAOLi;
            end    
            ...
            % forces in the ligaments
                % magnitudes
            magn_SAOL(i) = s_SAOL(i)*CSA_SAOL(i);
            magn_dAOL(i) = s_dAOL(i)*CSA_dAOL(i);
    ...
    Ttot_lig(i,:) = T_SAOL(i,:) + T_dAOL(i,:) + T_DRL(i,:) + T_POL(i,:) + T_UCL(i,:);
        ...
        % CARTILAGE
            ...
            for j=1:length(CA_F_trap)
                ...
                % stress at P1
                stressP1_mc1(j) = 1/4*HA*(1 + d1*(Defmc1_P1(j) - 1))*1/Defmc1_P1(j)*(Defmc1_P1(j).^2 - 1/(Defmc1_P1(j).^2))+exp(Cc*(Defmc1_P1(j).^2 - 3));
                stressP1_trap(j) = 1/4*HA*(1 + d1*(Deftrap_P1(j) - 1))*1/Deftrap_P1(j)*(Deftrap_P1(j).^2 - 1/(Deftrap_P1(j).^2))+exp(Cc*(Deftrap_P1(j).^2 - 3));
                stressP1(j) = stressP1_mc1(j) + stressP1_trap(j);
                % stress at P2
                stressP2_mc1(j) = 1/4*HA*(1 + d1*(Defmc1_P2(j) - 1))*1/Defmc1_P2(j)*(Defmc1_P2(j).^2 - 1/(Defmc1_P2(j).^2))+exp(Cc*(Defmc1_P2(j).^2 - 3));
                stressP2_trap(j) = 1/4*HA*(1 + d1*(Deftrap_P2(j) - 1))*1/Deftrap_P2(j)*(Deftrap_P2(j).^2 - 1/(Deftrap_P2(j).^2))+exp(Cc*(Deftrap_P2(j).^2 - 3));
                stressP2(j) = stressP2_mc1(j) + stressP2_trap(j);
                % stress at P3
                stressP3_mc1(j) = 1/4*HA*(1 + d1*(Defmc1_P3(j) - 1))*1/Defmc1_P3(j)*(Defmc1_P3(j).^2 - 1/(Defmc1_P3(j).^2))+exp(Cc*(Defmc1_P3(j).^2 - 3));
                stressP3_trap(j) = 1/4*HA*(1 + d1*(Deftrap_P3(j) - 1))*1/Deftrap_P3(j)*(Deftrap_P3(j).^2 - 1/(Deftrap_P3(j).^2))+exp(Cc*(Deftrap_P3(j).^2 - 3));
            ...
            % calculate the reaction force matrix for each triangle of the new F matrix
                for j=1:length(CA_F_trap)
                    ...
                    % matrix containing the corresponding vector forces
                    F_cart(j,:) = dir_F(j,:)*magn_carti(j);

                % calculate the corresponding reaction torque matrix (N.mm)
                    MA_cart(j,:) = [OF(j,1)-CP(i,1) OF(j,2)-CP(i,2) OF(j,3)-CP(i,3)];
    ...
    Rot_x(i) = Tt(i,1)*10^-3/I1cr*dt;
    Rot_y(i) = Tt(i,2)*10^-3/I2cr*dt;
    Rot_z(i) = Tt(i,3)*10^-3/I3cr*dt;
    % (degrees)
    ...
    if SD(i) < d_crit
        message = 'stopped due to bone-to-bone contact'
        break
    end
        % if the total torque gets close to 0 (equilibrium reached)
    if abs(Tt(i,1)) < 1 && abs(Tt(i,2)) < 1 && abs(Tt(i,3)) < 1
        message = 'equilibrium reached'
        break
    end
        % if the variation of the total torque gets too high (system getting unstable)
    if length(t)>1
        if abs(Tt(i,1)-Tt(i-1,1)) > 500 || abs(Tt(i,2)-Tt(i-1,2)) > 500 || abs(Tt(i,3)-Tt(i-1,3)) > 500
            message = 'stopped due to a large variation in torque between the last time steps (system unstable)'
            break
        end
    end
end
...
for j=1:length(CA_F_trap)
    ...
    % stress at P1
    stressP1_mc1_init(j) = 1/4*HA*(1 + d1*(Defmc1_P1_init(j) - 1))*1/Defmc1_P1_init(j)*(Defmc1_P1_init(j).^2 - 1/(Defmc1_P1_init(j).^2))+exp(Cc*(Defmc1_P1_init(j).^2 - 3));
    stressP1_trap_init(j) = 1/4*HA*(1 + d1*(Deftrap_P1_init(j) - 1))*1/Deftrap_P1_init(j)*(Deftrap_P1_init(j).^2 - 1/(Deftrap_P1_init(j).^2))+exp(Cc*(Deftrap_P1_init(j).^2 - 3));
    stressP1_init(j) = stressP1_mc1_init(j) + stressP1_trap_init(j);
    % stress at P2
    stressP2_mc1_init(j) = 1/4*HA*(1 + d1*(Defmc1_P2_init(j) - 1))*1/Defmc1_P2_init(j)*(Defmc1_P2_init(j).^2 - 1/(Defmc1_P2_init(j).^2))+exp(Cc*(Defmc1_P2_init(j).^2 - 3));
    stressP2_trap_init(j) = 1/4*HA*(1 + d1*(Deftrap_P2_init(j) - 1))*1/Deftrap_P2_init(j)*(Deftrap_P2_init(j).^2 - 1/(Deftrap_P2_init(j).^2))+exp(Cc*(Deftrap_P2_init(j).^2 - 3));
    stressP2_init(j) = stressP2_mc1_init(j) + stressP2_trap_init(j);
    % stress at P3
    stressP3_mc1_init(j) = 1/4*HA*(1 + d1*(Defmc1_P3_init(j) - 1))*1/Defmc1_P3_init(j)*(Defmc1_P3_init(j).^2 - 1/(Defmc1_P3_init(j).^2))+exp(Cc*(Defmc1_P3_init(j).^2 - 3));
    stressP3_trap_init(j) = 1/4*HA*(1 + d1*(Deftrap_P3_init(j) - 1))*1/Deftrap_P3_init(j)*(Deftrap_P3_init(j).^2 - 1/(Deftrap_P3_init(j).^2))+exp(Cc*(Deftrap_P3_init(j).^2 - 3));
