...
for i=1:length(CA_F_trap)
    ...
    CG = mean(Vor_mc1); % centroid of the MC1
    dcg_cr = sqrt((PC(1)-CG(1))^2+(PC(2)-CG(2))^2+(PC(3)-CG(3))^2); % distance from CG to CR
...
for i=1:imax
    ...
    % Total torque due to cartilage deformation (N.mm)
        ...
        % Transposition of inertia from center of gravity to point of contact
        CG = mean(Vi_mc1); % centroid of the MC1
        dcg_cr = sqrt((PC(i,1)-CG(1))^2+(PC(i,2)-CG(2))^2+(PC(i,3)-CG(3))^2); % distance from CG to CR
        I1cr = I1 + M_mc1*dcg_cr;
        I2cr = I2 + M_mc1*dcg_cr;
        I3cr = I3 + M_mc1*dcg_cr;
