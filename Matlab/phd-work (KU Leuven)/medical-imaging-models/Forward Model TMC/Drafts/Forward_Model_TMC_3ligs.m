...
origin_lig_available = 0;       % loads data if available (=1), else, generate data
...
if(origin_lig_available)
    ...
    idxDO2 = C(5,1);
    idxDO3 = C(6,1);
    O_AOL_i = V_trap(C(4,1),:);
    O_DRL_i = V_trap(C(5,1),:);
    O_IML_i = V_trap(C(6,1),:);
else
    [ad_curve] = PlacePoints3({F_trap}, {V_trap}, {F_trap}, {V_trap}, 'Select the origin of the AOL, DRL (IN THIS ORDER). End by pressing enter and exiting the figure');
    close;
    % Retrieves coordinates of the points:
    ...
    idxO = [idxDO1 0 0; idxDO2 0 0; idxDO3 0 0];
    
    % Save in a txt file
    fid = fopen(fileO_lig,'w+');
    fprintf(fid,'%5.8f\t%5.8f\t%5.8f\r\n',[O_AOL,O_DRL,O_IML...
        idxO(1,:),idxO(2,:),idxO(3,:)]);
    fclose(fid);
end
OPs = [O_AOL_i;O_DRL_i;O_IML_i];
...
if(insertion_lig_available)
    ...
    idxDI2 = D(5,1);
    idxDI3 = D(6,1);
    I_AOL_i = Vor_mc1(D(4,1),:);
    I_DRL_i = Vor_mc1(D(5,1),:);
    I_IML_i = Vor_mc1(D(6,1),:);

else
    [ad_curve] = PlacePoints3({F_mc1}, {Vor_mc1}, {F_mc1}, {Vor_mc1}, 'Select the insertion of the AOL, DRL, IML, IN THIS ORDER. End by pressing enter and exiting the figure');
    close;
    % Retrieves coordinates of the points:
    ...
    idxI = [idxDI1 0 0; idxDI2 0 0; idxDI3 0 0];
    
    % Save in a txt file
    fid = fopen(fileI_lig,'w+');
    fprintf(fid,'%5.8f\t%5.8f\t%5.8f\r\n',[I_AOL,I_DRL,I_IML...
        idxI(1,:),idxI(2,:),idxI(3,:)]);
    fclose(fid);
end
IPs = [I_AOL_i;I_DRL_i;I_IML_i];
...
for i=1:tmax*10
    ...
    Ttot_lig(i,:) = T_AOL(i,:) + T_DRL(i,:) + T_IML(i,:);
    ...
    if abs(Tt(i,1)) > 1000 && abs(Tt(i,2)) > 1000 && abs(Tt(i,3)) > 1000
        message = 'Code stopped: too high torques - constantly increasing -> unstable system'
        break
    end
...
f7=figure;
plot(t,magn_AOL,'r',t,magn_DRL,'b',t,magn_IML,'g');
xlabel('time (s)');
ylabel('Tension in the ligaments (N)')
legend('AOL','DRL','IML','Location','NorthEastOutside')
