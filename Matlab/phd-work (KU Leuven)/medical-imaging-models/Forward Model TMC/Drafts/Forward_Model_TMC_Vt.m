...
M_mc1 = 0.002; % in kg
% Principal moments of inertia (1: x; 2: y; 3: z), in kg.m2
I1 = 4.241*10^(-8);
I2 = 4.13*10^(-7);
I3 = 4.198*10^(-7);
...
z_angle = 0;
    ...
    % Ligament properties
AL = 1; % average cross-sectional area of thumb ligaments (in mm2)
C1 = 1; % in MPa
C2 = 1; % in MPa
C3 = 33; % in MPa
nu1 = 1; % in MPa.s
...
f3 = figure;
    ...
    % Plots the mc1 CS
    hold on
    quiver3(O_mc1(1),O_mc1(2),O_mc1(3),40*Xaxis_mc1(1),40*Xaxis_mc1(2),40*Xaxis_mc1(3),...
        'r','LineWidth',5,'MaxHeadSize',0.3,'AutoScale','off');
    quiver3(O_mc1(1),O_mc1(2),O_mc1(3),10*Yaxis_mc1(1),10*Yaxis_mc1(2),10*Yaxis_mc1(3),...
        'g','LineWidth',5,'MaxHeadSize',0.3,'AutoScale','off');
    quiver3(O_mc1(1),O_mc1(2),O_mc1(3),10*Zaxis_mc1(1),10*Zaxis_mc1(2),10*Zaxis_mc1(3),...
        'b','LineWidth',5,'MaxHeadSize',0.3,'AutoScale','off');
...
if(origin_lig_available)
    ...
    idxDO4 = C(9,1);
    idxDO5 = C(10,1);
    O_SAOL_idx = V_trap(C(6,1),:);
    O_dAOL_idx = V_trap(C(7,1),:);
    O_DRL_idx = V_trap(C(8,1),:);
    O_POL_idx = V_trap(C(9,1),:);
    O_UCL_idx = V_trap(C(10,1),:);
else
    ...
    O_SAOL_idx = V_trap(idxDO1,:);
    O_dAOL_idx = V_trap(idxDO2,:);
    O_DRL_idx = V_trap(idxDO3,:);
    O_POL_idx = V_trap(idxDO4,:);
    O_UCL_idx = V_trap(idxDO5,:);
...
OPs = [O_SAOL_idx;O_dAOL_idx;O_DRL_idx;O_POL_idx;O_UCL_idx];
...
if(insertion_lig_available)
    ...
    idxDI4 = D(9,1);
    idxDI5 = D(10,1);
    I_SAOL_idx = Vor_mc1(D(6,1),:);
    I_dAOL_idx = Vor_mc1(D(7,1),:);
    I_DRL_idx = Vor_mc1(D(8,1),:);
    I_POL_idx = Vor_mc1(D(9,1),:);
    I_UCL_idx = Vor_mc1(D(10,1),:);
    ...
    I_SAOL_idx = Vor_mc1(idxDI1,:);
    I_dAOL_idx = Vor_mc1(idxDI2,:);
    I_DRL_idx = Vor_mc1(idxDI3,:);
    I_POL_idx = Vor_mc1(idxDI4,:);
    I_UCL_idx = Vor_mc1(idxDI5,:);
...
% Calculate the centroid of the ligaments insertion point:
IPs = [I_SAOL_idx;I_dAOL_idx;I_DRL_idx;I_POL_idx;I_UCL_idx];
CR = centroid(IPs);

% Plot the five insertion point with the centroid for visualization
f5=figure;
for i=1:5;
    plot3(IPs(i,1),IPs(i,2),IPs(i,3),'Marker','o','MarkerSize',10,'Color','b','MarkerFaceColor','b');
    arrow([IPs(i,1);IPs(i,2);IPs(i,3)],[CR(1);CR(2);CR(3)],...
        10,70,30,5,'EdgeColor','r','FaceColor','r');
    hold on
end
hold on
plot3(CR(1),CR(2),CR(3),'Marker','o','MarkerSize',10,'Color','k','MarkerFaceColor','k');
hold on
for i=1:4;
    arrow([IPs(i,1);IPs(i,2);IPs(i,3)],[IPs(i+1,1);IPs(i+1,2);IPs(i+1,3)],...
        10,10,10,5,'EdgeColor','b','FaceColor','b');
end
hold on
arrow([IPs(5,1);IPs(5,2);IPs(5,3)],[IPs(1,1);IPs(1,2);IPs(1,3)],...
        10,10,10,5,'EdgeColor','b','FaceColor','b');
...
% FINAL SYSTEM
    ...
    % Calculate the axis of rotation:
        % The rotation axis is defined as the axis that is orthogonal to
        % the 2 vectors connecting CR to the point of application of the
        % force and CR to the origin of the force. It is then also
        % orthogonal to the planes created by CR, PointA and Vorigin
    
    direct1 = PointA_idx - CR / norm(PointA_idx - CR);
    direct2 = Vorigin1 - CR / norm(Vorigin1 - CR);
    direct3 = cross(direct1,direct2) / norm(cross(direct1,direct2));
    direct1 = cross(direct2,direct3) / norm(cross(direct2,direct3));
    
    rot_axis = direct3;
    
    % Plots the corresponding rotation axis:
    hold on
    quiver3(CR(1),CR(2),CR(3),40*rot_axis(1),40*rot_axis(2),40*rot_axis(3),...
        'b','LineWidth',5,'MaxHeadSize',0.3,'AutoScale','off');
    quiver3(CR(1),CR(2),CR(3),-40*rot_axis(1),-40*rot_axis(2),-40*rot_axis(3),...
        'b','LineWidth',5,'MaxHeadSize',0.3,'AutoScale','off');
...
% INITIAL CONDITIONS
    ...
    % Transposition of inertia from center of gravity to center of rotation
    CG = polyhedronCentroid(Vor_mc1,F_mc1); % center of gravity of MC1
    dcg_cr = sqrt((CR(1)-CG(1))^2+(CR(2)-CG(2))^2+(CR(3)-CG(3))^2); % distance from CG to CR
    I1cr = I1 - M_mc1*dcg_cr;
    I2cr = I2 - M_mc1*dcg_cr;
    I3cr = I3 - M_mc1*dcg_cr;
    
    % Calculation of the resultant torque (N.mm)
    MA = [PointA_idx(1) - CR(1);PointA_idx(2) - CR(2); PointA_idx(3) - CR(3)]; % Moment arm
    TR = [MA(2)*FR(3) - MA(3)*FR(2);MA(3)*FR(1) - MA(1)*FR(3);MA(1)*FR(2) - MA(2)*FR(1)];
    
    % Calculation of the initial length of the ligaments
    linit_SAOL = norm(I_SAOL_idx-O_SAOL_idx);
    linit_dAOL = norm(I_dAOL_idx-O_dAOL_idx);
    linit_DRL = norm(I_DRL_idx-O_DRL_idx);
    linit_POL = norm(I_POL_idx-O_POL_idx);
    linit_UCL = norm(I_UCL_idx-O_UCL_idx);
...
for i=1:length(CA_F_trap)
    ...
    for i=1:length(CA_F_trap)
        MA_cart(i,:) = [CR(1)-OF(i,1) CR(2)-OF(i,2) CR(3)-OF(i,3)];
...
for i=1:tmax
        t(i)=i/tmax;    % time (s)
        dt = 1/tmax;    % time step delta t (s)
        % LIGAMENTS        
            % NEW ORIGINS COORDINATES FOR LIGAMENTS
            O_SAOL_i = Vor_mc1(idxDO1,:);
            O_dAOL_i = Vor_mc1(idxDO2,:);
            O_DRL_i = Vor_mc1(idxDO3,:);
            O_POL_i = Vor_mc1(idxDO4,:);
            O_UCL_i = Vor_mc1(idxDO5,:);
            OPsi = [O_SAOL_i;O_dAOL_i;O_DRL_i;O_POL_i;O_UCL_i];
            % NEW INSERTIONS COORDINATES FOR LIGAMENTS
            ...
            % NEW COORDINATES FOR THE CENTER OF ROTATION            
            CR(i,:) = centroid(IPsi);
            
            % new length of the ligaments
            l_SAOL(i) = norm(I_SAOL_i-O_SAOL_idx);
            l_dAOL(i) = norm(I_dAOL_i-O_dAOL_idx);
            l_DRL(i) = norm(I_DRL_i-O_DRL_idx);
            l_POL(i) = norm(I_POL_i-O_POL_idx);
            l_UCL(i) = norm(I_UCL_i-O_UCL_idx);
            ...
            % forces in the ligaments
                % magnitudes
            magn_SAOL(i) = stot_SAOL(i)*AL;
            magn_dAOL(i) = stot_dAOL(i)*AL;
            magn_DRL(i) = stot_DRL(i)*AL;
            magn_POL(i) = stot_POL(i)*AL;
            magn_UCL(i) = stot_UCL(i)*AL;
                % directions
    ...
    % Total torque from ligaments stretch
    Ttot_lig(i,:) = T_SAOL(i,:) + T_dAOL(i,:) + T_DRL(i,:) + T_POL(i,:) + T_UCL(i,:);
        ...
        % CARTILAGE
            ...
            [idx_cart,D_cart] = knnsearch(V_trap,Vor_mc1);
            ...
            for j=1:length(CA_F_trap)
                ...
                % need to know the index of the closest point in the V matrix of the
                % MC1
                Di_trap_mc1_P1 = pdist2(V_trap(CA_F_trap(j,1),:),Vor_mc1);
                [minDiP1(j),idxP1(j)] = min(Di_trap_mc1_P1);
                Di_trap_mc1_P2 = pdist2(V_trap(CA_F_trap(j,2),:),Vor_mc1);
                [minDiP2(j),idxP2(j)] = min(Di_trap_mc1_P2);
                Di_trap_mc1_P3 = pdist2(V_trap(CA_F_trap(j,3),:),Vor_mc1);
                [minDiP3(j),idxP3(j)] = min(Di_trap_mc1_P3);
                % distance between each vertices for P1, P2 and P3 of the F matrix
                DiP1(j) = ((V_trap(CA_F_trap(j,1),1)-Vor_mc1(idxP1(j),1)).^2+...
                    (V_trap(CA_F_trap(j,1),2)-Vor_mc1(idxP1(j),2)).^2+...
                    (V_trap(CA_F_trap(j,1),3)-Vor_mc1(idxP1(j),3)).^2).^(1/2);
                DiP2(j) = ((V_trap(CA_F_trap(j,2),1)-Vor_mc1(idxP2(j),1)).^2+...
                    (V_trap(CA_F_trap(j,2),2)-Vor_mc1(idxP2(j),2)).^2+...
                    (V_trap(CA_F_trap(j,2),3)-Vor_mc1(idxP2(j),3)).^2).^(1/2);
                DiP3(j) = ((V_trap(CA_F_trap(j,3),1)-Vor_mc1(idxP3(j),1)).^2+...
                    (V_trap(CA_F_trap(j,3),2)-Vor_mc1(idxP3(j),2)).^2+...
                    (V_trap(CA_F_trap(j,3),3)-Vor_mc1(idxP3(j),3)).^2).^(1/2);
                % deformation in MC1 cartilage layer for P1, P2 and P3 of the F matrix
            ...
            % calculate the reaction force matrix for each triangle of the new F matrix
                ...
                % calculate the corresponding reaction torque matrix (N.mm)
                    MA_cart(j,:) = [CR(i,1)-OF(j,1) CR(i,2)-OF(j,2) CR(i,3)-OF(j,3)];
    ...
    % TOTAL TORQUE AT TIME T
    Tt(i,1) = TR(1) + Ttot_lig(i,1) + Ttot_carti(i,1);
    Tt(i,2) = TR(2) + Ttot_lig(i,2) + Ttot_carti(i,2);
    Tt(i,3) = TR(3) + Ttot_lig(i,3) + Ttot_carti(i,3);
    
    if abs(Tt(i,1)) < 0.001 && abs(Tt(i,2)) < 0.001 && abs(Tt(i,3)) < 0.001
        break
    else
    
    % ANGULAR VELOCITY
    Rot_x(i) = Tt(i,1)/I1*t(i);
    Rot_y(i) = Tt(i,2)/I2*t(i);
    Rot_z(i) = Tt(i,3)/I3*t(i);
    ...
    Vor_mc1 = Roti_mc1*Vor_mc1.' + repmat(Tr_mc1,1,size(Vor_mc1,1));
    Vor_mc1 = Vor_mc1.';
    end
end

% FINAL INDEXES FOR LIGAMENTS        
    % FINAL ORIGINS COORDINATES FOR LIGAMENTS
    O_SAOL_i = Vor_mc1(idxDO1,:);
    O_dAOL_i = Vor_mc1(idxDO2,:);
    O_DRL_i = Vor_mc1(idxDO3,:);
    O_POL_i = Vor_mc1(idxDO4,:);
    ...
    I_UCL_i = Vor_mc1(idxDI5,:);
    IPsi = [I_SAOL_i;I_dAOL_i;I_DRL_i;I_POL_i;I_UCL_i];    
    % FINAL COORDINATES FOR THE CENTER OF ROTATION            
    CRf = centroid(IPsi);
...
title('Evolution of the tension in the ligaments');
...
title('Evolution of the maximal and average contact stress');
...
title('Evolution of the projected contact area');

% Plots the system in its final/resting position    
    f10 = figure;
    [obj, li, ax] = GUI_PlotShells(f10, {F_trap;F_mc1},{V_trap;Vor_mc1},...
        {ones(size(V_trap,1),1);ones(size(Vor_mc1,1),1)});
    box off
    hold on
    for i=1:5;
        arrow([OPsi(i,1);OPsi(i,2);OPsi(i,3)],[IPsi(i,1);IPsi(i,2);IPsi(i,3)],...
        10,70,30,5,'EdgeColor',[1 0.5 0.2],'FaceColor',[1 0.5 0.2])
        plot3(OPsi(i,1),OPsi(i,2),OPsi(i,3),'Marker','o','MarkerSize',10,'Color',...
        'r','MarkerFaceColor','r');
