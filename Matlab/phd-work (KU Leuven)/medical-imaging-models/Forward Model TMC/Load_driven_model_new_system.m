...
dir = 'C:\Users\u0096636\Documents\Professional\PhD KU Leuven\Data\Forward Model\STL\Scan3\';
...
dt = 0.00001;    % time step delta t (s)
threshold = 1.03;

    % Loading conditions    
        % Define the magnitude of the force vector:
magnitude_x = 0.1; % in N
magnitude_y = 0;
magnitude_z = -0.5;
...
%   origin: midway between the two landmarks defining the y-axis.
%   y: extends from the exact mid-point of the central ridge of the 
%       MC1 and follows the main axis of the MC1.
%   x: runs in a dorsal-to-volar direction along a line perpendicular to
%       the central ridge of the MC1 and passes through the palmar beak.
%   z: is perpendicular to the X and Y axes and nearly parallel to the
%       central ridge of the trapezial metacarpal surface.
...
if(mc1_cs_available)
    ...
    Xmc11 = B(1,:);    % central dorsal tip of the MC1 articular surface
    Xmc12 = B(2,:);    % palmar beak of the MC1 articular surface
    Ymc1 = B(3,:);     % center of the proximal articular surface, facing the center of the base of trapezium
else
    ...
    close all;
    % Retrieves coordinates of the points:
    Xmc11 = ad_curve.points{end,1}(1,:);
    Xmc12 = ad_curve.points{end,1}(2,:);
    Ymc1 = ad_curve.points{end,1}(3,:);
...
% Find the indexes of the vertices used to define the MC1 CS:
    [idxXmc11,DXmc11] = knnsearch(Xmc11,V_mc1);
    [CXmc11,idxXmc11] = min(DXmc11);
    [idxXmc12,DXmc12] = knnsearch(Xmc12,V_mc1);
    [CXmc12,idxXmc12] = min(DXmc12);
    [idxYmc1,DYmc1] = knnsearch(Ymc1,V_mc1);
    [CYmc1,idxYmc1] = min(DYmc1);
    
O_mc1 = (Xmc11+Xmc12)/2;
Xaxis_mc1 = (Xmc12-Xmc11)/norm(Xmc12-Xmc11);
Yaxis_mc1 = (O_mc1- Ymc1)/norm(O_mc1-Ymc1);
...
% Convert mc1 into MC1 CS
V_mc1_mc1cs = R_glob2loc_mc1*V_mc1.' + repmat(T_glob2loc_mc1,1,size(V_mc1,1));
V_mc1_mc1cs = V_mc1_mc1cs.';

% Plots the MC1 with the new coordinate system
    fmc1 = figure;
    [obj, li, ax] = GUI_PlotShells(fmc1, {F_mc1},{V_mc1_mc1cs},...
        {ones(size(V_mc1_mc1cs,1),1)});
    box off
        % Plots the coordinate system and its origin
    hold on
    arrow([0;0;0],[15;0;0],10,70,30,5,'EdgeColor','r','FaceColor','r');
...
if ans == 0
    close(f1)
    error('Incorrect correct system. Please try again!');
    break
end
end
    
if x_translation ~= 0 && y_translation ~= 0 && z_translation ~= 0 && x_angle ~= 0 && y_angle ~= 0 && z_angle ~= 0;
    % Plots the system in the Trapezium CS
    f3 = figure;
...
for i=1:imax
        t(i)=i*dt;    % time (s)
        iteration = i        
        % ROTATION MATRICES (between global and local frames)
        % Retrieve coordinates of each landmark in the current frame:
        Xmc11 = Vi_mc1(idxXmc11,:);
        Xmc12 = Vi_mc1(idxXmc12,:);
        Ymc1 = Vi_mc1(idxYmc1,:);
...
if i == 1
    syms Rx Ry Rz
w1 = Rx/dt + Rz/dt*sin(Ry);
w2 = Ry/dt*cos(Rx) - Rz/dt*sin(Rx)*cos(Ry);
w3 = Rz/dt*cos(Rx)*cos(Ry);
w_prime = R01*[w1; w2; w3];
L1 = I1*w_prime(1);
L2 = I2*w_prime(2);
L3 = I3*w_prime(3);
L_prime = [L1; L2; L3];
a1 = Rx/dt^2 ...
    + Rz/dt^2*sin(Ry) ...
    + Ry/dt*Rz/dt*cos(Rx)^2*cos(Ry);
a2 = Ry/dt^2*cos(Rx) ...
    - Rz/dt^2*sin(Rx)*cos(Ry) ...
    - Rx/dt*Rz/dt*cos(Rx)*cos(Ry);
a3 = Rz/dt^2*cos(Rx)*cos(Ry) ...
    - Rx/dt*Rz/dt*sin(Rx)*cos(Ry) ...
    - Ry/dt*Rz/dt*cos(Rx)*sin(Ry);
a_prime = R01*[a1; a2; a3];
system = R10*(I*a_prime + cross(w_prime,L_prime));
[solRx, solRy, solRz] = vpasolve([Tt(i,1)*10^-3 == system(1), Tt(i,2)*10^-3 == system(2), Tt(i,3)*10^-3 == system(3)],[Rx,Ry,Rz]);

Rot_x(i) = double(solRx);
Rot_y(i) = double(solRy);
Rot_z(i) = double(solRz);

elseif i ==2
    syms Rx Ry Rz
w1 = (Rx-Rot_x(i-1))/dt + (Rz-Rot_z(i-1))/dt*sin(Ry);
w2 = (Ry-Rot_y(i-1))/dt*cos(Rx) - (Rz-Rot_z(i-1))/dt*sin(Rx)*cos(Ry);
w3 = (Rz-Rot_z(i-1))/dt*cos(Rx)*cos(Ry);
w_prime = R01*[w1; w2; w3];
L1 = I1*w_prime(1);
L2 = I2*w_prime(2);
L3 = I3*w_prime(3);
L_prime = [L1; L2; L3];
a1 = (Rx-Rot_x(i-1))/dt^2 - Rot_x(i-1)/dt^2 ...
    + ((Rz-Rot_z(i-1))/dt^2 - Rot_z(i-1)/dt^2)*sin(Ry) ...
    + (Ry-Rot_y(i-1))/dt*(Rz-Rot_z(i-1))/dt*cos(Rx)^2*cos(Ry);
a2 = ((Ry-Rot_y(i-1))/dt^2 - Rot_y(i-1)/dt^2)*cos(Rx) ...
    - ((Rz-Rot_z(i-1))/dt^2 - Rot_z(i-1)/dt^2)*sin(Rx)*cos(Ry) ...
    - (Rx-Rot_x(i-1))/dt*(Rz-Rot_z(i-1))/dt*cos(Rx)*cos(Ry);
a3 = ((Rz-Rot_z(i-1))/dt^2 - Rot_z(i-1)/dt^2)*cos(Rx)*cos(Ry) ...
    - (Rx-Rot_x(i-1))/dt*(Rz-Rot_z(i-1))/dt*sin(Rx)*cos(Ry) ...
    - (Ry-Rot_y(i-1))/dt*(Rz-Rot_z(i-1))/dt*cos(Rx)*sin(Ry);
a_prime = R01*[a1; a2; a3];
system = R10*(I*a_prime + cross(w_prime,L_prime));
[solRx, solRy, solRz] = vpasolve([Tt(i,1)*10^-3 == system(1), Tt(i,2)*10^-3 == system(2), Tt(i,3)*10^-3 == system(3)],[Rx,Ry,Rz]);

Rot_x(i) = double(solRx) - Rot_x(i-1);
Rot_y(i) = double(solRy) - Rot_y(i-1);
Rot_z(i) = double(solRz) - Rot_z(i-1);
...
L3 = I3*w_prime(3);
L_prime = [L1; L2; L3];
a1 = (Rx-Rot_x(i-1))/dt^2 - (Rot_x(i-1)-Rot_x(i-2))/dt^2 ...
    + ((Rz-Rot_z(i-1))/dt^2 - (Rot_z(i-1)-Rot_z(i-2))/dt^2)*sin(Ry) ...
    + (Ry-Rot_y(i-1))/dt*(Rz-Rot_z(i-1))/dt*cos(Rx)^2*cos(Ry);
a2 = ((Ry-Rot_y(i-1))/dt^2 - (Rot_y(i-1)-Rot_y(i-2))/dt^2)*cos(Rx) ...
    - ((Rz-Rot_z(i-1))/dt^2 - (Rot_z(i-1)-Rot_z(i-2))/dt^2)*sin(Rx)*cos(Ry) ...
    - (Rx-Rot_x(i-1))/dt*(Rz-Rot_z(i-1))/dt*cos(Rx)*cos(Ry);
a3 = ((Rz-Rot_z(i-1))/dt^2 - (Rot_z(i-1)-Rot_z(i-2))/dt^2)*cos(Rx)*cos(Ry) ...
    - (Rx-Rot_x(i-1))/dt*(Rz-Rot_z(i-1))/dt*sin(Rx)*cos(Ry) ...
    - (Ry-Rot_y(i-1))/dt*(Rz-Rot_z(i-1))/dt*cos(Rx)*sin(Ry);
a_prime = R01*[a1; a2; a3];
system = R10*(I*a_prime + cross(w_prime,L_prime));
...
Rot_z(i) = double(solRz) - Rot_z(i-1);
    ...
    Rot_deg_acc_x(i) = sum(Rot_deg_x);
    Rot_deg_acc_y(i) = sum(Rot_deg_y);
    Rot_deg_acc_z(i) = sum(Rot_deg_z);
    ...
    if abs(Tt(i,1)) < threshold && abs(Tt(i,2)) < threshold && abs(Tt(i,3)) < threshold
...
% Plots the evolution of the rotation angles
f15=figure;
plot(t,Rot_deg_acc_x,'r',t,Rot_deg_acc_y,'g',t,Rot_deg_acc_z,'b',t,t*0,'k');
xlabel('Time (s)');
ylabel('Rotation angle (degrees)')
legend('Rot_x','Rot_y','Rot_z','Location','NorthEastOutside')
