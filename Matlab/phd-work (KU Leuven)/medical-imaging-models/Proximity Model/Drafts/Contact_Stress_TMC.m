...
% FILES
Bone1 = 'SCAN20_N_mc1.stl';
Bone2 = 'SCAN20_N_trap.stl';
...
for i=1:min(length(idxD1),length(idxD2))
    ...
    Def_mc1(i) = (Ttot-dist_m1(i))/2/Tmc1+1;
    Def_trap(i) = (Ttot-dist_m1(i))/2/Ttrap+1;
    ...
    colrstl1(idxD1(i),:)= ...
        [-((stress1(i)-HA)^2)/((HA)^2)+1,...
        -((stress1(i)-HA/2)^2)/((-HA/2)^2)+1,...
        -(stress1(i)^2)/((HA)^2)+1];
    if stress1(i) >= HA
...
for i=1:min(length(idxD3),length(idxD4))
    ...
    Def3_mc1(i) = (Ttot-dist_m2(i))/2/Tmc1+1;
    Def3_trap(i) = (Ttot-dist_m2(i))/2/Ttrap+1;
    ...
    stress2(i) = stress2_mc1(i) + stress2_trap(i);
        colrstl2(idxD3(i),:)= ...
        [-((stress2(i)-HA)^2)/((HA)^2)+1,...
        -((stress2(i)-HA/2)^2)/((-HA/2)^2)+1,...
        -(stress2(i)^2)/((HA)^2)+1];
    if stress2(i) >= HA
