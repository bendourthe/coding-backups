...
if(trap_cs_available)
    ...
    [F_Trap_P1, V_Trap_P1] =  STL_ReadFile([dir Trap_P1],true);
    [ad_curve] = PlacePoints3({F_Trap_P1}, {V_Trap_P1}, {F_Trap_P1}, {V_Trap_P1}, 'Select 3 landmarks (Y1, Y2 and X, in that order!). End by pressing enter and exiting the figure');
    close all;
...
RotMat = [Xaxis_trap', Yaxis_trap', Zaxis_trap'];
loc2glob = [RotMat, O_trap';
     0 0 0 1]; 
   % These tranformation matrices convert from local to global CS.
   % Invert them to go from global to local.
glob2loc = [RotMat', -(RotMat')*O_trap'; 0 0 0 1];
