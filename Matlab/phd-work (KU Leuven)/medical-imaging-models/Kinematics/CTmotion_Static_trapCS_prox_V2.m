...
% Folder with files
dir = 'C:\Users\u0096636\Documents\Professional\PhD KU Leuven\Data\Study_Brown_Rig\Kinematics\Tests\'; 
...
V_Trap_P1 = R_glob2loc*V_Trap_P1.' + repmat(T_glob2loc,1,size(V_Trap_P1,1));
V_Trap_P1 = V_Trap_P1.';
V_MC1_P1 = R_glob2loc*V_MC1_P1.' + repmat(T_glob2loc,1,size(V_MC1_P1,1));
V_MC1_P1 = V_MC1_P1.';

V_Trap_P2 = R_glob2loc*V_Trap_P2.' + repmat(T_glob2loc,1,size(V_Trap_P2,1));
V_Trap_P2 = V_Trap_P2.';
V_MC1_P2 = R_glob2loc*V_MC1_P2.' + repmat(T_glob2loc,1,size(V_MC1_P2,1));
V_MC1_P2 = V_MC1_P2.';
...
for i = 1:length(CA_V_Bone1prox)
    ...
    [minDCAprox1(i),idxCAprox1(i)] = min(DCAprox1);
        ...
        test_F1 = ismember(F_MC1_P1,idxD1prox);
        ...
        for i=1:length(idx_F1)
            CA_F_Bone1prox(i,:) = F_MC1_P1(idx_F1(i),:);
        end
...
for i = 1:length(CA_V_Bone3prox)
    ...
    [minDCAtprox(i),idxCAprox2(i)] = min(DCAtprox);
        ...
        % We can now test to which triangle these vertices correspond in
        % the F matrix and save their indexes
        test_F2 = ismember(F_MC1_P12,idxD3prox);
        ...
        for i=1:length(idx_F2)
            CA_F_Bone2prox(i,:) = F_MC1_P12(idx_F2(i),:);
        end

%       Each triangle is composed of 3 points called a,b and c
Xa3 = V_Trap_P12(CA_F_Bone2prox(:,1),1);
Ya3 = V_Trap_P12(CA_F_Bone2prox(:,1),2);
Za3 = V_Trap_P12(CA_F_Bone2prox(:,1),3);

Xb3 = V_Trap_P12(CA_F_Bone2prox(:,2),1);
Yb3 = V_Trap_P12(CA_F_Bone2prox(:,2),2);
Zb3 = V_Trap_P12(CA_F_Bone2prox(:,2),3);

Xc3 = V_Trap_P12(CA_F_Bone2prox(:,3),1);
Yc3 = V_Trap_P12(CA_F_Bone2prox(:,3),2);
Zc3 = V_Trap_P12(CA_F_Bone2prox(:,3),3);
...
for i = 1:length(CA_V_Bone1proxb)
    ...
    [minDCAprox1b(i),idxCAprox1b(i)] = min(DCAprox1b);
        ...
        % We can now test to which triangle these vertices correspond in
        % the F matrix and save their indexes
        test_F3 = ismember(F_Trap_P2,idxD1proxb);
        ...
        for i=1:length(idx_F3)
            CA_F_Bone1proxb(i,:) = F_Trap_P2(idx_F3(i),:);
        end
...
% We can now test to which triangle these vertices correspond in
        % the F matrix and save their indexes
        test_F4 = ismember(F_Trap_P22,idxD3proxb);
        ...
        for i=1:length(idx_F4)
            CA_F_Bone3proxb(i,:) = F_Trap_P22(idx_F4(i),:);
        end
