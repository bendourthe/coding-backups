function [power, intensity, tot_intensity, freq_spectrum, pa] = emg_wavelet_act(signal, ...
    sampling_rate, scale, control, nr_wavelets, threshold)
%% DESCRIPTION:
%
% [power, intensity, tot_intensity, freq_spectrum] = 
%    emg_wavelet(signal,sampling_rate, new_sampling_rate, scale, control, nr_wavelets)
%
% Applies a wavelet filter on an EMG signal and calculates the corresponding
%   power, then calculates which portions of the signal are active and
%   calculate the corresponding intensity, total intensity, and frequency spectrum
%
...
%% Function
    ...
    idxAct = find(mean(power') > threshold*max(mean(power')));
    startAct = min(idxAct);
    endAct = max(idxAct);
