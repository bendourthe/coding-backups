...
%% Signal information
dataFull = read_data(pathName,'\1_101_emg_1.emg',4);
...
%% Filtering parameters
    ...
    lff = 4;      % Low frequency factor: increasing it will filter higher frequency data
    lf_mode = 1;
    lf_type = 4;
...
for i=1:includedSteps
    ...
    clf = 200;          % Calculates the cutting low frequency
    ...
    % calculated location of each heel strikes(red star)
        ...
        timeRaw = 0+1/sampling_frequency:1/sampling_frequency:length(rawSignal)/sampling_frequency;
        timeFiltered = 0+1/sampling_frequency:1/sampling_frequency:length(filteredSignal)/sampling_frequency;
    ...
    hold on
    plot(heelStrikeLoc/sampling_frequency,heelStrikeVal,'r*','MarkerSize',10);
    xlim([0 max(timeRaw)])
