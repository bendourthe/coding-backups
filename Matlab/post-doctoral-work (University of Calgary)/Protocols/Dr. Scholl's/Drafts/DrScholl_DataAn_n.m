...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Window definition
    w1 = 400;   % how much time (ms) before heel strike
    w2 = 300;   % how much time (ms) after heel strike
...
%% TOTAL INTENSITY of GM during SUSTAINED CONTRACTION (50% trials)
    ...
    for i=1:size(halfp,1)
        ...
        [p50p, int50p, tInt50p, f50p] = emg_wavelet(emgData, samplingFrequency, samplingFrequency, 0.3, 3, 13);
    ...
    for i=1:size(emg,1)
        ...
        [numSteps, frameLength, firstPeakIdx, filteredSignal, heelStrikeLoc] = heel_strike_loc_temp(dataAcc, samplingFrequency);
        
        % Crops each signal to fit the filtered accelerometer signal
        [cropDataAcc,reshapedDataAcc] = crop_signal_step(dataAcc, numSteps, frameLength, firstPeakIdx, ...
                                                                heelStrikeLoc, w1, w2, samplingFrequency);
        [cropEmgDataGM,reshapedEmgDataGM] = crop_signal_step(emgDataGM, numSteps, frameLength, firstPeakIdx, ...
                                                                heelStrikeLoc, w1, w2, samplingFrequency);
        [cropEmgDataVL,reshapedEmgDataVL] = crop_signal_step(emgDataVL, numSteps, frameLength, firstPeakIdx, ...
                                                                heelStrikeLoc, w1, w2, samplingFrequency);
        [cropEmgDataVM,reshapedEmgDataVM] = crop_signal_step(emgDataVM, numSteps, frameLength, firstPeakIdx, ...
                                                                heelStrikeLoc, w1, w2, samplingFrequency);
        ...
        [pGM, intGM, tIntGM, fGM] = emg_wavelet(meanEmgDataGM, samplingFrequency, samplingFrequency, scale, control, numWavelets);
        [pVL, intVL, tIntVL, fVL] = emg_wavelet(meanEmgDataVL, samplingFrequency, samplingFrequency, scale, control, numWavelets);
        [pVM, intVM, tIntVM, fVM] = emg_wavelet(meanEmgDataVM, samplingFrequency, samplingFrequency, scale, control, numWavelets);
    ...
    for i=1:size(emg,1)
        ...
        emgDataVL = data(:,3);
        emgDataVM = data(:,4);
        [pGM, intGM, tIntGM, fGM] = emg_wavelet(emgDataGM, samplingFrequency, samplingFrequency, scale, control, numWavelets);
        [pVL, intVL, tIntVL, fVL] = emg_wavelet(emgDataVL, samplingFrequency, samplingFrequency, scale, control, numWavelets);
        [pVM, intVM, tIntVM, fVM] = emg_wavelet(emgDataVM, samplingFrequency, samplingFrequency, scale, control, numWavelets);
