function [numSteps, frameLength, heelStrikeIdx] = heel_strike_loc(signal, sampling_frequency, hff, lff)
%% DESCRIPTION:
%
...
%   - hff: high frequency factor: default 1 means 100% of the cutting
%           frequency that is automatically calculated
%           (decreasing it will filter lower frequency data)
%   - lff: low frequency factor: default 1 means 100% of the cutting
%           frequency that is automatically calculated
%           (increasing it will filter higher frequency data)
%
%% Output:
...
%% STEP 3: Wavelet filtering of each frame for heel strike detection
    ...
    hff = 1;   % High frequency factor: default 1 means 100% of the cutting frequency that is automatically calculated
               % (decreasing it will filter lower frequency data)
    lff = 2;   % Low frequency factor: default 1 means 100% of the cutting frequency that is automatically calculated
               % (increasing it will filter higher frequency data)
    ...
    for i=1:numSteps-1
        ...
        lf_signal_crop = lf_signal(I1-50:I1);
        range = mean(lf_signal)+1*std(lf_signal);   % Defines a threshold to detect actual peaks
        [A,idx]=find(lf_signal_crop>range);         % Finds the datapoints that are above of the threshold
        [B,idx2]=findpeaks(lf_signal_crop(idx));    % Finds the indexes of the peaks that are above the threshold
        B(1)
        lf_signal
        [C,idx3]=find(lf_signal==B(1));             % Finds the index of the heel strike peak (first peak)
