...
%         emg_name = [emg1; emg2; emg3; emg4; emg5; emg6; emg7; emg8];
...
%% Currex Static
    ...
    if currexS == 0
        ...
        % Arch Index
        for i=startSession:size(currexStat_name,1)
            current_trial = i
            % Load data
    ...
    if currexD == 0
        ...
        % Arch Index
        for i=startSession:size(currexDyn_name,1)
            current_trial = i
            % Load data
...
%% MVC Trials
    ...
    for i=startSession:size(mvc_name,1)
        current_trial = i
...
%% 50% Trials
    ...
    % Wavelet filtering    
    for i=startSession:size(half_names,1)        
                % Load data
...
%% Walking Trials
    ...
    % Processing
    for i=startSession:size(emg_name,1)
        ...
        accMin = reshape(acc(2:end),samplingFrequency*60,walkTime)';
        emgGMMin = reshape(emgGM(2:end),samplingFrequency*60,walkTime)';
        emgVLMin = reshape(emgVL(2:end),samplingFrequency*60,walkTime)';
        emgVMMin = reshape(emgVM(2:end),samplingFrequency*60,walkTime)';
...
plot(meanEmgGM)
            ...
            mnfGM(i,j) = fGM*paGM.cfs'/sum(fGM);
            mnfVL(i,j) = fVL*paVL.cfs'/sum(fVL);
            mnfVM(i,j) = fVM*paVM.cfs'/sum(fVM);
        ...
        % Reorganizes the values in a table (ROW = trial, COLUMN = session)
        mnfGM = reshape(mnfGM,numSessions,walkTime).';    
        mnfVL = reshape(mnfVL,numSessions,walkTime).';        
        mnfVM = reshape(mnfVM,numSessions,walkTime).';
