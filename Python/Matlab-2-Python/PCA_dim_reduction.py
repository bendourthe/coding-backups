...
Output:
    ...
    - acc_explained: vector showing the accumulative contribution of each PC to
    the total variance (last row should be 100)
...
def PCA_dim_reduction(data , variance):
...
# STEP 5 - Variance ---------------------------------------------------------- #            
    ...
    for i in range(1 , np.len(explained)):
        acc_explained[i] = np.sum(explained[1:i])
    
    # Identify how many PCs are needed to reach the corresponding threshold
    for i in range(1, np.len(acc_explained)):
        if acc_explained[i] >= variance:
            num_PCs = i
    ...
    data_PCs = zip(*weights[2:num_PCs(1)+1 , :])
