...
# ---------------------------------------------------------------------------- #
'''
[eigen_vectors eigen_values weights] = SVD_residual(data,num_eigenvectors)

   Computes the eigenvectors and eigenvalues using a Singular Value
   Decomposition (SVD) method on the data.
   Compared to a regular SVD, this function only keeps the rows of weights 
   that matches the number of dimensions.

Author: Benjamin Dourthe

Created: September 15th, 2018
Last update: September 15th, 2018

Input: 
   - data: m x n matrix containing data, where m = trials/subjects/observations
   and n = number of variables
   - num_eigenvectors: defines the desired number of eigenvectors (write 'max'
   if you wish to retain all of them)
